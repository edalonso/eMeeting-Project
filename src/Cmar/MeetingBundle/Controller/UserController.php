...
class UserController extends Controller
{
     ...
     *
     * @Route("/recording_private/{id}", name="index_recording", requirements = {"id" = "\d+"})
     ...
     *
     * @Route("/recording/{secretsalt}", name="index_public_recording")
     * @Template("CmarMeetingBundle:Room:room.html.twig")
     **/
    public function recordingPublicAction($secretsalt)
    {
        $em = $this->getDoctrine()->getEntityManager();
        $repo = $em->getRepository('CmarMeetingBundle:Recording');
        $recording = $repo->findOneRecordingBySecretSalt($secretsalt);
        ...
        } elseif ($recording->getState() == Recording::STATE_PRIVATE) {
            return $this->redirect($this->generateUrl('index_recording', array('id' => $recording->getId())));
        } else {
            return array(
                         'url' => $recording->getUrl(),
                         'title' => $recording->getTitle()
    ...
    /**
     * Safe title of Recording
     *
     * @Route("/safe_title_recording", name="safe_title_recording")
     * @Template("CmarMeetingBundle:User:recordingList.html.twig")
     * @Method("post")
     **/
    public function safeTitleRecordingAction()
    {
        ...
        $em = $this->getDoctrine()->getEntityManager();
        $repo = $em->getRepository('CmarMeetingBundle:Recording');
        $recording = $repo->find($request->request->get("id_recording"));

        if (!$recording) {
            throw $this->createNotFoundException('Unable to find Recording entity.');
        }

        $recording->setTitle(trim($request->request->get("caja_recording")));
    ...
    public function immediateAction()
    {
        ...
        try {
            ...
            $this->get('session')->setFlash('error', 'Select other title this one is being used by another user');
            return $this->redirect($this->generateUrl('index'));
        } catch (\Exception $e) {
            try {
    ...
    /**
     * Change eMeeting to users can record or non can record.
     *
     * @Route("/canrecord/{canrecord}/{id}", name="can_record", requirements = {"id" = "\d+"})
     *
     */
    public function changeToRecord($id, $canrecord = false)
    {
        $em = $this->getDoctrine()->getEntityManager();
        $repo = $em->getRepository('CmarMeetingBundle:Meeting');
        $meetingService = $this->get('cmar_meeting');
        ...
        if ($canrecord) {
            $meeting->setCanRecord(true);
        } else {
            $meeting->setCanRecord(false);
        }
     ...
     *
     * @Route("/locked_record/{locked}/{secretsalt}", name="locked_recording")
     * @Template("CmarMeetingBundle:User:recordingList.html.twig")
     */
    public function lockedRecordingAction($secretsalt, $locked = false)
    {
        $em = $this->getDoctrine()->getEntityManager();
        $recording = $em->getRepository('CmarMeetingBundle:Recording')->findOneRecordingBySecretSalt($secretsalt);

        if (!$recording) {
            throw $this->createNotFoundException('Unable to find Recording entity.');
        }
        if ($locked==0) {
            $recording->setState(Recording::STATE_PRIVATE);
            $em->persist($recording);
        } elseif ($locked==1){
    ...
    public function adminAction()
    {
        ...
        if (!$month_log = \DateTime::createFromFormat('Y-m-d H:i:s', $string_month . '-01 00:00:00')){//Evitamos que cuenta mal por desfase horario
            throw $this->createNotFoundException('Date bad format');
        }
        
        if (!$month_log_total = \DateTime::createFromFormat('Y-m-d H:i:s', $string_month . '-01 00:00:00')){//Evitamos que cuenta mal por desfase horario
            throw $this->createNotFoundException('Date bad format');
            }

        $em = $this->getDoctrine()->getEntityManager();

        $logs = $em->getRepository('CmarMeetingBundle:Log')->findByMonth($month_log);
        $logs_total = $em->getRepository('CmarMeetingBundle:Log_Total')->findByMonth($month_log_total);
    ...
    /**
     * Graphics Bars App to Admin
     *
     * @Route("/admin_bars", name="admin_bars")
     */
    public function adminActionBars(Request $request)
    {
        $month_log = $request->request->get("month_log");
        $month_log_total = $request->request->get("month_log_total");
        ...
        if ($month_log == null) {
            ...
            $fecha_esp = str_replace("/", "-", $month_log);
            $string_month = strtotime($fecha_esp);
            $string_month = date('Y-m', $string_month);
            $month_log = \DateTime::createFromFormat('Y-m-d H:i:s', $string_month . '-01 00:00:00');

            $interval = new \DateInterval("P1M");
            $month_log->sub($interval);//Restamos un mes a la fecha   
        } else {
            $fecha_esp = str_replace("/", "-", $month_log);
            $string_month = strtotime($fecha_esp);
            $string_month = date('Y-m', $string_month);
            ...
            $interval = new \DateInterval("P1M");
            $month_log->add($interval);//Sumamos un mes a la fecha
        }

        if ($month_log_total == null) {
            $now = new \DateTime("now");
            $string_month = $now->format('Y-m');
            ...
            $fecha_esp = str_replace("/", "-", $month_log_total);
            $string_month = strtotime($fecha_esp);
            $string_month = date('Y-m', $string_month);
            $month_log_total = \DateTime::createFromFormat('Y-m-d H:i:s', $string_month . '-01 00:00:00');

            $interval = new \DateInterval("P1M");
            $month_log_total->sub($interval);//Restamos un mes a la fecha
        } else {
            $fecha_esp = str_replace("/", "-", $month_log_total);
            $string_month = strtotime($fecha_esp);
            $string_month = date('Y-m', $string_month);
            ...
            $interval = new \DateInterval("P1M");
            $month_log_total->add($interval);//Sumamos un mes a la fecha
        } 
        ...
        return new Response(json_encode(array( 'data' => $data_log, 'data_total' => $data_log_total)));
    }
}
